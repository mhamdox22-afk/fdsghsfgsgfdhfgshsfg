...
const changePwd = () => {
  ...
  const password = form.value.newPwd;
  const numReg = /^[0-9]*$/
  const numRe = new RegExp(numReg)
  const isPureNumber = !numRe.test(password);
  if (isPureNumber) {
    // 如果是纯数字，显示错误信息
    _toast('password_num')
    return
  ...
  let params = `oldPwd=${form.value.oldPwd}&newPwd=${form.value.newPwd}&signType=1`
  updateFundPwd(params).then(res => {
    if (res.code == '200') {
      // showToast('资金密码修改成功')
